document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function(e) {
            ...
            if (targetElement) {
                ...
                // Auto-focus first input of the contact form once scrolling completes
                if (href === '#contact') {
                    setTimeout(() => {
    ...
    function drawEcosystemLines() {
        ...
        // Draw connections from the Navigator™ card (spans Row 1) down to the satellite cards:
        // All paths flow downward ('bottom') in a spinal neural network
        // 1. Navigator -> Student Intelligence
        createPath('intel', coords.nav.left + 125, coords.nav.bottom, coords.intel.left + 90, coords.intel.cy, 'var(--accent)', 'bottom');
        // 2. Navigator -> Life Skills
        createPath('life', coords.nav.left + 125, coords.nav.bottom, coords.life.left + 90, coords.life.cy, '#f59e0b', 'bottom');
        // 3. Navigator -> Future Skills
        createPath('future', coords.nav.left + 125, coords.nav.bottom, coords.future.left + 90, coords.future.cy, '#10b981', 'bottom');
        // 4. Navigator -> Skill Labs
        createPath('labs', coords.nav.left + 125, coords.nav.bottom, coords.labs.left + 90, coords.labs.cy, '#8b5cf6', 'bottom');
        // 5. Navigator -> School Transformation
        createPath('school', coords.nav.left + 125, coords.nav.bottom, coords.school.left + 90, coords.school.cy, 'var(--accent)', 'bottom');

        let ecosystemPaths = [];
        window.ecosystemPaths = ecosystemPaths;
        ...
        function createPath(nodeId, x1, y1, x2, y2, color, direction) {
            ...
            // Register paths for dynamic scroll binding
            setTimeout(() => {
                try {
                    const length = pathBg.getTotalLength();
                    pathBg.style.strokeDasharray = length;
                    pathBg.style.strokeDashoffset = length;
                    pathGlow.style.strokeDasharray = length;
                    pathGlow.style.strokeDashoffset = length;
                    ...
                    pathBg.style.strokeDasharray = "none";
                    pathGlow.style.strokeDasharray = "none";
                }
            }, 50);
        }
    }

    // Scroll listener for drawing paths dynamically
    function updateEcosystemScrollPaths() {
        const paths = window.ecosystemPaths;
        ...
        const enter = viewHeight;
        const exit = -rect.height;
        const current = rect.top;
        
        const totalRange = enter - exit;
        const currentPos = enter - current;
        let progress = Math.max(0, Math.min(1, currentPos / totalRange));
        
        // Active draw phase between scroll threshold
        let drawProgress = Math.max(0, Math.min(1, (progress - 0.25) / 0.35));
        
        paths.forEach(item => {
            if (!item.pathBg || !item.pathGlow) return;
            const offset = item.length * (1 - drawProgress);
            item.pathBg.style.strokeDashoffset = offset;
            
            if (!item.pathGlow.classList.contains('path-active')) {
                item.pathGlow.style.strokeDashoffset = offset;
            } else {
                item.pathGlow.style.strokeDashoffset = 0;
            }
        });
    }

    window.addEventListener('scroll', updateEcosystemScrollPaths);
    window.addEventListener('resize', updateEcosystemScrollPaths);
    ...
    // SCROLL-TRIGGERED HIGH-CONTRAST NODE HIGHLIGHTS IN "WHY PARTNER WITH US"
    const featureBlocks = document.querySelectorAll('.partnership-feature-block');
    if ('IntersectionObserver' in window && featureBlocks.length > 0) {
        const milestoneObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                const milestoneNum = entry.target.getAttribute('data-milestone');
                const svgNode = document.getElementById(`node-milestone-${milestoneNum}`);
                if (entry.isIntersecting) {
                    entry.target.classList.add('in-focus');
                    if (svgNode) svgNode.classList.add('is-active');
                } else {
                    const rect = entry.target.getBoundingClientRect();
                    // Remove highlight only if scrolled out completely
                    if (rect.top > window.innerHeight || rect.bottom < 0) {
                        entry.target.classList.remove('in-focus');
        ...
        featureBlocks.forEach(block => {
            milestoneObserver.observe(block);
        });
    }
});
